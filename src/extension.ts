...
class YoutubeIntegration implements vscode.WebviewViewProvider {
    ...
    webviewView.webview.onDidReceiveMessage((message) => {
      ...
      switch (message.command) {
        ...
        case "API_KEY_DETECTED":
          ...
          const musicVideosURL =
            nextPageToken.trim().length <= 0
              ? `videos?part=snippet&chart=mostPopular&maxResults=10&key=${this._token}`
              : `videos?part=snippet&chart=mostPopular&maxResults=10&key=${this._token}`;
          const entertainmentVideosURL =
            nextPageToken.trim().length <= 0
              ? `videos?part=snippet&chart=mostPopular&maxResults=10&key=${this._token}`
              : `videos?part=snippet&chart=mostPopular&maxResults=10&key=${this._token}`;

          const technologyVideosURL =
            nextPageToken.trim().length <= 0
              ? `videos?part=snippet&chart=mostPopular&maxResults=10&key=${this._token}`
              : `videos?part=snippet&chart=mostPopular&maxResults=10&key=${this._token}`;

          const gamingVideosURL =
            nextPageToken.trim().length <= 0
              ? `videos?part=snippet&chart=mostPopular&maxResults=10&key=${this._token}`
              : `videos?part=snippet&chart=mostPopular&maxResults=10&key=${this._token}`;
          ...
          if (desiredURL === "ERROR") throw "An error occured";
          customAxios
            .get(desiredURL)
            .then((res) => {
              ...
              let sendResourceToWebView = this.preprocess(
  ...
  private preprocess(items: any): any {
    //preprocess given item and only include relevant information
    let preprocessedItem: any = [];
    for (let i = 0; i < items.length; i++) {
      preprocessedItem.push(
        new Object({
